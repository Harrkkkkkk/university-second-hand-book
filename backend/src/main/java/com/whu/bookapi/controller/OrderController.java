...
public class OrderController {
    ...
    public OrderController(OrderService orderService, UserService userService, BookService bookService, com.whu.bookapi.service.CartService cartService) {
    ...
    public ResponseEntity<?> pay(@RequestHeader(value = "token", required = false) String token,
        ...
        if ("paid".equals(existing.getStatus())) {
             return ResponseEntity.status(HttpStatus.CONFLICT).body(java.util.Map.of("message", "订单已支付"));
        }
