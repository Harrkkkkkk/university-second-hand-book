# 校园二手书交易系统前端部署指南

## 部署步骤

### 第1步: 修改API配置

#### 1.1 修改API代理配置
在本机 `校园二手书/vite.config.js` 中修改：
```javascript
import { defineConfig } from 'vite'
import vue from '@vitejs/plugin-vue'
import path from 'path'

export default defineConfig({
  plugins: [vue()],
  resolve: {
    alias: {
      '@': path.resolve(__dirname, './src')
    }
  },
  server: {
    port: 3000,
    proxy: {
      '/book-api': {
        target: 'http://121.36.98.215:8080', // 公网地址
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/book-api/, '/book-api')
      }
    }
  }
})
```





### 第2步: 构建生产版本

在本机上执行
```bash
# 在/校园二手书/目录下执行
npm install
npm run build
```

构建完成后，会在 `/校园二手书/dist/` 目录下生成生产文件。

### 第3步: 部署到云服务器

#### 3.1 git clone项目到云服务器
```bash
# 在云服务器/home/book-platform目录下执行
git clone https://gitee.com/aoae0/book.git
```

#### 3.2 复制构建文件
```bash
# 在云服务器上执行
mkdir -p /var/www/html/book-platform
cp -r dist/* /var/www/html/book-platform/
```

### 第4步: 配置Nginx反向代理

#### 4.1 安装Nginx
```bash
# CentOS/RHEL
yum install -y nginx


#### 4.2 配置Nginx
创建配置文件 `/etc/nginx/conf.d/book-platform.conf`：

```bash
cat > /etc/nginx/conf.d/book-platform.conf << 'EOF'
server {
    listen 80;
    server_name ecs公网ip;
    
    # 前端静态文件
    location / {
        root /var/www/html/book-platform;
        index index.html;
        try_files $uri $uri/ /index.html;
        
        # 处理Vue Router的history模式
        location ~* \.(js|css|png|jpg|jpeg|gif|ico|svg)$ {
            expires 1y;
            add_header Cache-Control "public, immutable";
        }
    }
    
    # API代理到后端服务
    location /book-api/ {
        proxy_pass http://localhost:8080/book-api/;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        
        # 跨域处理
        add_header Access-Control-Allow-Origin *;
        add_header Access-Control-Allow-Methods 'GET, POST, PUT, DELETE, OPTIONS';
        add_header Access-Control-Allow-Headers 'Content-Type, Authorization, X-Requested-With';
        
        # 处理OPTIONS请求
        if ($request_method = 'OPTIONS') {
            add_header Access-Control-Allow-Origin *;
            add_header Access-Control-Allow-Methods 'GET, POST, PUT, DELETE, OPTIONS';
            add_header Access-Control-Allow-Headers 'Content-Type, Authorization, X-Requested-With';
            add_header Access-Control-Max-Age 1728000;
            return 204;
        }
    }
}
EOF
```
###设置正确的文件权限
```bash
# 设置目录所有者
chown -R nginx:nginx /var/www/html/book-platform

# 设置目录权限为 755
find /var/www/html/book-platform -type d -exec chmod 755 {} \;

# 设置文件权限为 644
find /var/www/html/book-platform -type f -exec chmod 644 {} \;
```

#### 4.3 启动Nginx
```bash
# 启动服务
systemctl start nginx
systemctl enable nginx

# 重载配置
nginx -t && systemctl reload nginx
```

### 第5步: 配置防火墙

#### 5.1 开放80端口
```bash
firewall-cmd --permanent --add-port=80/tcp
firewall-cmd --reload

```

#### 5.2 检查服务状态
```bash
systemctl status nginx
netstat -tlnp | grep 80
```

## 🌐 访问测试